 ...
 */
function resize(input, output, options) {
  ...
  result.forEach(async item => {
    ...
    const data = await image.resize({
      width:Number(options.width) || metadata.width,
      height:Number(options.height) || metadata.height,
      fit:options.fit,
